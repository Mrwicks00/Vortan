...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    if (filters.baseToken !== "All") {
      filtered = filtered.filter(
        (project) => project.baseToken === filters.baseToken
      );
    }
      ...
      <div className="space-y-6">
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Discover and participate in the most innovative token launches
            across the galaxy
          </p>
        </div>
        ...
        {error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="glass-effect glow-border rounded-lg p-6 space-y-4"
              >
        ...
        <div className="text-center text-sm text-muted-foreground">
          Showing{" "}
          {Array.isArray(filteredProjects) ? filteredProjects.length : 0} of{" "}
